...
async function hasNpm() {
  try {
    await execSync('npmjs --version', { stdio: 'ignore' });
    return true;
  } catch {
    return false;
  }
...
async function setup() {
  try {
    ...
    // Remove extra files
    fs.unlinkSync(path.join(appPath, 'bin', 'createStructure.js'));
    fs.rmdirSync(path.join(appPath, 'bin'));
    if (!useNpm) {
      fs.unlinkSync(path.join(appPath, 'package.lock'));
    }
    ...
    console.log(useNpm ? '    yarn dev' : '    npm run dev');
    console.log();
